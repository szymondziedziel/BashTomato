...
function uid_dump_window_hierarchy() {
  device_id=`default $1 ''`
  dump_xml_file_name=`default $2 temporary_xml_dump.xml`

  dumppath=`adb -s $device_id shell uiautomator dump | cut -d ' ' -f5`
...
function uid_take_screenshot() {
  device_id=`default $1 ''`
  screenshot_file_name=`default $2 temporaty_screenshot_file_name.png`
