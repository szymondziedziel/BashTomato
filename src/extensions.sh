...
function ext_screenshot_node() { # makes screenshot and cuts out image of given element
  ...
  local screenshot_filename=$(default "$2" "$TEMPORARY_SCREENSHOT_FILENAME") # path to full screenshot with '.png' extension from which node screenshot will be taken
  local node_screenshot_filename=`default "$3" "$TEMPORARY_NODE_SCREENSHOT_FILENAME"` # path to point to where node screenshot will be stored
...
function ext_inspect_window_hierarchy() { # makes full screenshot and then images of all nodes in hierarchy (very poor performance)
  ...
  local screenshot_filename=$(default "$3" "$TEMPORARY_SCREENSHOT_FILENAME") # path to full screenshot with '.png' extension from which node screenshot will be taken
  local directory=$(default "$4" "$(mktemp -d)") # directory path where to store all nodes' screenshots
